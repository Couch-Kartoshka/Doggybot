"""
Модуль Telegram-бота, отправляющего изображения с собаками пользователю.

Отправление изображений осуществляется после запроса пользователя
на данное действие через кнопку "Подай собакена!".

В случае, если API 'thedogapi' будет недоступен, предусмотрена возможность
отправки изображений с кошками через API 'thecatapi'.
"""
...
DEFAULT_ENDPOINT = 'https://api.thedogapi.com/v1/images/search'
BACKUP_ENDPOINT = 'https://api.thecatapi.com/v1/images/search'

# Настройка интерфейса бота:
CAT_BOT_NAME = '@kisunyabot'
BUTTONS = ['Подай собакена!', 'Мне бы котика']
...
def redirect_to_bot(update: CustomUpdate, context: CustomContext) -> None:
    ...
    logger.debug(f'Получено сообщение "{update.message.text}"')
    chat = update.effective_chat
    text = (f'Боюсь, что это не ко мне. Но вот мой друг {CAT_BOT_NAME} '
            'обязательно поможет.')
    send_message(context=context, chat_id=chat.id, text=text)
...
def wake_up(update: CustomUpdate, context: CustomContext) -> None:
    ...
    text = f'Привет, {name}. Посмотри, какого собакена я тебе нашёл!'
    send_message(context=context, chat_id=chat.id, text=text)
